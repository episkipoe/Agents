...
void attempt_to_mate(void) {
	if(organism_of_focus==worldPort) { /*I can't see anybody, mate with self*/
		...
		if(myGenome.transcribeGene("//asexual_reproduction", 4, portstr.str().c_str(), genome_file, child_file.str().c_str(), worldportstr.str().c_str()) > -1) {
			const char * msg = "I think I'm a clone now";
			send_message(myPort, worldPort, SOUND, strlen(msg), msg);
		}
...
int check_for_message(int sock) {
	...
	if(type == SOUND) {
		...
		if(sender == worldPort) { /*mate directive*/
			...
			stringstream child_file;  child_file<<genome_file<<n_children;
			if(myGenome.transcribeGene("//sexual_reproduction", 5, portstr.str().c_str(), genome_file, mate_genome, child_file.str().c_str(), worldportstr.str().c_str()) > -1) {
				string msg = "I just mated with " + sender;
				send_message(myPort, worldPort, SOUND, msg.length(), msg.c_str());
			}
		}
...
int main (int argc, char * argv[]) {
	...
	myGenome.transcribeGene("//red_appearance", 1, portstr.str().c_str());
	...
	while(1) {
		try {
			...
			if(myNet.subnetFires("reproduction", 0.33)) attempt_to_mate();

			map<string,unsigned char*>::iterator output_iterator;
			for( output_iterator = output_data.begin() ; output_iterator != output_data.end() ; output_iterator++) {
				delete [] output_iterator->second;
			}
